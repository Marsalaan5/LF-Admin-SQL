...
const RolesManagement = () => {
  ...
  const formatPermissions = (permissions) => {
    return Object.entries(permissions).map(([component, actions]) => {
    ...
    <div className="container mt-5">
      <h2 className="text-center mb-4">Role Management</h2>
       <div className="mb-3 text-end">
        <button className="btn btn-success" onClick={openAddModal}>Add Role</button>
      </div>
          ...
          {roles.map((role, index) => (
            <tr key={role.id}>
              ...
              <td className="text-center">
                <button className="btn btn-warning btn-sm mx-1" onClick={() => handleEdit(role)}>Edit</button>
                <button className="btn btn-danger btn-sm mx-1" onClick={() => handleDelete(role.id)}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
                        ...
                        {modules[component].map(action => (
                          <div key={action} className="form-check form-check-inline">
                            <input
                              ...
                              id={`${component}-${action}`}
                            />
                            <label className="form-check-label" htmlFor={`${component}-${action}`}>
                              {action}
                            </label>
                          </div>
                        ))}
